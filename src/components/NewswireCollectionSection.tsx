...
export default function NewswireCollectionSection({ title }: Props) {
    ...
    const handleCollectionClick = (collectionName: string) => {
        ...
        router.push(`/newswire/article/${sanitizedCollectionName}`);
    };
    return (
        <>
            <MainContainer className="py-4 md:py-8">
                ...
                <div className="-mx-4 mt-3 sm:mx-0">
                    ...
                    >
                        ...
                        {NEWSWIRE_ITEMS?.map((newswireItem: any, key: any) => (
                            <SwiperSlide key={key}>
                                <div key={key}
                                    onClick={() => handleCollectionClick(newswireItem.title)}
                                ...
                                >
                                    ...
                                    <div className='p-3'>
                                        <div className="mt-4 flex justify-between">
                                            <p className='font-bold text-slate-900 dark:text-gray-100 '>{newswireItem.title}</p>
                                            <p className='text-sm font-semibold text-slate-900 dark:text-gray-400'>{newswireItem.release_date}</p>
                                        </div>
                                        {/* <div className='text-sm'>
                                            <p className='font-light text-slate-900 dark:text-gray-300  '>
                                                {newswireItem.description}
                                            </p>
                                        </div> */}
                                    </div>
                                </div>
